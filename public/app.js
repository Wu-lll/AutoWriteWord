...
function markdownToHtml(markdown) {
  if (!markdown) {
    return "<p class='empty'>正文会显示在这里。</p>";
  }
...
function buildExportHtml(title, bodyHtml) {
  const cnFont = getComputedStyle(document.documentElement).getPropertyValue("--reader-cn-font").trim() || '"STSong", "SimSun", serif';
  return `<!doctype html>
<html lang="zh-CN">
  <head>
    <meta charset="utf-8">
    ...
    <style>
      body {
        ...
        background: #f5efe6;
        color: #2b241d;
        font-family: "Times New Roman", ${cnFont}, serif;
      }
      .page {
      ...
      h1 { text-align: center; font-size: 34px; margin: 0 0 28px; }
      h2 { font-size: 25px; margin: 24px 0 16px; }
      h3 { font-size: 21px; margin: 20px 0 12px; }
      p { margin: 0 0 18px; line-height: 2; text-indent: 2em; font-size: 18px; }
      ul { margin: 0 0 18px 1.6em; }
      li { margin-bottom: 10px; line-height: 1.9; font-size: 18px; }
...
function renderPrompts(steps) {
  if (!steps?.length) {
    promptsBox.innerHTML = "<p class='empty'>生成后这里会显示每一步实际使用的提示词。</p>";
    return;
  }

  promptsBox.innerHTML = steps
    .map(step => `
...
function updateRunView(run) {
  ...
  const statusMap = {
    ...
    running: ["生成中", `当前步骤：${run.currentStep || "初始化"}，模式：${run.generationMode === "balanced" ? "标准" : "快速"}`],
    completed: ["完成", "四个阶段已经全部完成。"],
...
async function fetchHistory() {
  ...
  const data = await response.json();
  if (!response.ok) {
    throw new Error(data.error || "读取历史任务失败。");
  }
  return data.runs || [];
}
...
function connectEvents(runId) {
  ...
  ["snapshot", "run.started", "step.started", "step.completed", "run.completed", "run.failed"].forEach(
    eventName => {
      activeEventSource.addEventListener(eventName, refresh);
    }
  );
...
testConnectionButton.addEventListener("click", async () => {
  ...
  setConnectionState("is-loading", "测试中...", "正在发送一个最小 chat/completions 请求。");
  ...
  try {
    ...
    setConnectionState("is-success", "连接成功", data.message || "模型已正常响应。");
  } catch (error) {
    setConnectionState("is-error", "连接失败", error.message);
  } finally {
...
historyBox.addEventListener("click", async event => {
  ...
  if (downloadButton) {
    ...
    try {
      ...
      downloadText(`${runId}.txt`, run.finalMarkdown || "");
    } catch (error) {
      setStatus("失败", error.message);
    }
  }
...
form.addEventListener("submit", async event => {
  ...
  const payload = {
    ...
    generationMode: fields.generationMode.value,
  ...
  try {
    ...
    setStatus("失败", error.message);
    storyBox.innerHTML = `<pre>${escapeHtml(error.message)}</pre>`;
  } finally {
...
exportTxtButton.addEventListener("click", () => {
  ...
  if (!markdown) return;
  downloadText(`${buildExportBaseName()}.txt`, markdown);
});
